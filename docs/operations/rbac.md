...
| `convert_ssis_package`          | none                                                 | —                                  | —                                                              | Local generation only. With `translation_mode="aoai"` (or legacy `llm_translate=true`): needs **Cognitive Services OpenAI User** on the Azure OpenAI resource. With `translation_mode="host"` (in-session agent translates): no Azure OpenAI permissions required. Set `SSIS_ADF_NO_LLM=1` (P4-8) to forbid LLM calls regardless of args. See [script-task-translation.md](../conversion/script-task-translation.md). |
...
| `validate_deployer_rbac` (P5-12) | none (offline) **or** `Reader`                     | subscription                       | —                                                              | Offline mode compares supplied role names. Live mode lists role assignments and definitions for the selected principal. |
| `diff_estate` (P5-16)           | none                                                 | —                                  | —                                                              | Pure local file comparison. |
